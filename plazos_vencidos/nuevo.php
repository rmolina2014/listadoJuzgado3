  ...
  <section class="content">
   <div class="card card-secundary">
      ...
      <div class="card-body">
        ...
        <input type="hidden" name="actuaciones_id" id="actuaciones_id">

        <div class="row">
          <div class="col-sm-12">
            <div class="form-group">
              <label>Requerimientos</label>
              <textarea type="text" id="requerimiento" name="requerimiento" class="form-control">   </textarea>  
            </div>
          </div>
        </div>
 ...
 <script type="text/javascript">
  ...
  elInput.addEventListener('keyup', function(e)
  {
    ...
    if (keycode == 13)
    {
     ...
     //num_expediente =$("#kol2").val();//num_expediente.valor;
     //alert(num_expediente.valor);
     ...
     $.ajax({
          ...
          success: function(data)
          {
            ...
            {
              //alert(content);
              fila="<li class='nav-link'><font style='vertical-align: inherit;'>"+content[i].numero_expediente+" -- "+content[i].persona+" DNI: "+content[i].persona_dni+"  "+content[i].domicilio+" -- "+content[i].rol+" </font><span class='badge bg-waring float-right'> <button type='button' onclick='borrar("+content[i].id+");' class='btn btn-default btn-sm'><i class='far fa-trash-alt'></i></button> </li>";
               $('#tabla').append(fila);
               $('#persona_id').val(content[i].persona_id);
               $('#expediente_id').val(content[i].expediente_id);
               $('#actuaciones_id').val(content[i].actuaciones_id);
                //lista.push(content[i].persona);
             }
            }
          }
          });//fin ajax
